...
class EloquentProductSkuRepository implements ProductSkuRepositoryContract, ProductSkuStockRepositoryContract, ProductMixRepositoryContract, SubscribeSkuRepositoryContract
{
    ...
    protected function updateOrCreate($sku_data, $product_sku_id = null, $product_id = 0)
    {
        ...
        $sku->fill(
            [
                ...
                'product_row' => array_get($sku_data,'product_row',''),
                'product_double' => array_get($sku_data,'product_double',0),
            ]);
    ...
    {
        return Product::query()->with('skus',  'cats','info','images')->where('status', ProductProtocol::VAR_PRODUCT_STATUS_UP)->get();
        /*return Product::query()->with('skus', 'cats', 'info','images')->where('status', ProductProtocol::VAR_PRODUCT_STATUS_UP)->whereHas('groups', function ($query) {
