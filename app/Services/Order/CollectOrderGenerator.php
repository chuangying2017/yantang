...
class CollectOrderGenerator implements OrderGeneratorContract {
    ...
    public function confirm($temp_order_id)
    {
        ...
        $this->setOrderRepo(app()->make(CollectClientOrderRepository::class));
        $order = $this->orderRepo->createOrder($temp_order->toArray());
    ...
    public function updateSku( $temp_order_id, $skus ){
        $config = [
            GetSkuInfo::class,
    ...
    public function updateAddress($temp_order_id, $address)
    {
        $temp_order = $this->getTempOrder($temp_order_id);
     ...
     * @return TempOrder
     */
    public function pullTempOrder($temp_order_id)
    {
    ...
    {
        $user_id_with_collect_order = explode(',', $user_id);
        $user_id = $user_id_with_collect_order[0];
        $collect_order_id = $user_id_with_collect_order[1];
        $config = [
            GetSkuInfo::class,
            GetUserInfo::class,
            GetOrderAddress::class,
