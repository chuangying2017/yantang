...
//导出文件
    ...
    public static function downloadStationExcel($station_order){

        $station_order->load('skus', 'station', 'staff', 'user', 'order', 'order.promotions', 'tickets', 'tickets.coupon', 'redEnvelope', 'residence');

        $filename = '燕塘 -' . $station_order[0]->station->name . '' . date('Y-m-d',time()) . '.xls';

        foreach ($station_order as $key => $preorder) {
            $e_data['序号'] = $key+1;
            ...
            $e_data['配送员'] = $preorder['staff']['name'];
            $e_data['订单号'] = $preorder['order_no'];
            $e_data['姓名'] = $preorder['name'];
            $e_data['电话'] = $preorder['phone'];
            $e_data['地址'] = $preorder['address'];
            $e_data['商品详情'] = self::getPreorderSkusArray($preorder['skus']);
            $e_data['下单时间'] = $preorder['order']['created_at'];
            $e_data['配送状态'] = PreorderProtocol::status($preorder['status']);
            $e_data['起送日期'] = $preorder['start_time'];
            $e_data['配送周期'] = $preorder['weekday_type'] == 'all' ? '周一到周日' : '周一到周五';
            $e_data['配送时间'] = $preorder['daytime'] == 1 ? '上午': '下午';

            $e_preorders[$key] = $e_data;
        }
        return self::downExcel($e_preorders, $filename);
    }
}
